...
export class FormComponent {
  ...
  private setDashboard(dashboard: Dashboard) {
    ...
    this.temp.displayName = this.dashboard.displayName
      ? this.dashboard.displayName
      : '';
    this.temp.logoUrl = this.dashboard.logoUrl ? this.dashboard.logoUrl : '';
    this.temp.boards = this.dashboard.boards
      ? this.dashboard.boards.join(',')
      : '';
    this.temp.programIncrement = this.dashboard.programIncrement
      ? this.dashboard.programIncrement
      : '';
    this.temp.applications = this.dashboard.applications
      ? this.dashboard.applications.join(',')
      : '';
    this.temp.codeRepos = this.dashboard.codeRepos
      ? this.dashboard.codeRepos.join(',')
      : '';
    this.temp.adminUsers = this.dashboard.adminUsers
      ? this.dashboard.adminUsers
      : [];
    this.temp.gitRepos = this.dashboard.gitRepos
      ? this.dashboard.gitRepos.join(',')
      : '';
    ...
    this.dashboardFilteredList = this.aggregatedDashboardsCtrl.valueChanges.pipe(
      startWith(null),
      map((dashboard: string | null) =>
        this._filter(
          dashboard,
          this.dashboardList,
          this.temp.aggregatedDashboards
        )
      )
    );
    ...
    this.temp.teamMembers = this.dashboard.teamMembers
      ...
      : [];
    this.temp.lastVersion = this.dashboard.lastVersion
      ? this.dashboard.lastVersion
      : '';
    ...
    this.temp.urlAlerts = this.dashboard.urlAlerts
      ...
      : '';
    this.temp.urlAlertsAuthorization = this.dashboard.urlAlertsAuthorization
      ? this.dashboard.urlAlertsAuthorization
      : '';
  }

  private mirrorTempValues() {
    this.dashboard.displayName = this.temp.displayName.length
      ? this.temp.displayName.trim()
      : undefined;
    this.dashboard.logoUrl = this.temp.logoUrl.length
      ? this.temp.logoUrl.trim()
      : undefined;
    this.dashboard.boards = this.temp.boards.length
      ? this.temp.boards.split(',')
      : undefined;
    ...
    this.dashboard.lastVersion = this.temp.lastVersion.length
      ? this.temp.lastVersion.trim()
      : undefined;
    this.dashboard.slackTeam = this.temp.slackTeam.length
      ? this.temp.slackTeam.trim()
      : undefined;
    this.dashboard.urlAlerts = this.temp.urlAlerts.length
      ? this.temp.urlAlerts.trim()
      : undefined;
    this.dashboard.urlAlertsAuthorization = this.temp.urlAlertsAuthorization
      .length
      ? this.temp.urlAlertsAuthorization.trim()
      : undefined;
    if (!this.edit) {
  ...
  private saveColumns(dashboard: Dashboard) {
    ...
    for (let i = 0; i < this.MAX_COLUMNS; i++) {
      ...
      Array.from(document.getElementById(`col${i + 1}`).children).forEach(
        el => this.dashboard.columns[i][this.dashboard.columns[i].length] = el.id
      );
    }
  }
