...
public class ReviewServiceTests {
    ...
    public void getAverageRateByAppNamesTest() {
        ...
        final List<String> appsNames = new ArrayList<>();
        ...
        when(reviewRepository.getAverageRateByAppNamesAfterTimestamp(eq(appsNames), any()))
            .then((Answer<List<ApplicationDTO>>) invocation -> {
                final List<ApplicationDTO> stats = new ArrayList<>();
        ...
        final List<ApplicationDTO> appsByNames
    ...
    public void createReviewTest() {
        final Review review1 = createReview().setCommentId("123354");
        final Review review2 = createReview().setCommentId("24324324");

        final Iterable<Review> reviews = Arrays.asList(review1, review2);

        when(reviewRepository.saveAll(reviews)).thenReturn(reviews);

        final List<String> reviewIds = reviewService.saveAll(reviews);
    ...
    public void createFeedbackReviewTest() {
        ...
        final ReviewDTO savedReview = map(review);

        when(reviewRepository.save(any(Review.class))).thenReturn(review);

        final ReviewDTO reviewDTO = reviewService.saveApplicationReview(review.getAppname(), savedReview);
